...
const Header = () => {
  ...
  return (
    <header className="absolute top-0 flex w-full justify-between bg-slate-950 text-slate-950 dark:bg-slate-50 ">
      <Link
        to="/dash"
        className="ml-4 flex items-center gap-2 px-3 py-1 text-2xl font-semibold sm:text-3xl"
      >
        <FaUniversity className="m-1" />
        <h1 className="m-0 pr-1 font-spectral text-slate-950 decoration-slate-50 decoration-[3px] underline-offset-[3px] hover:underline">
          COLLEGE-HUB
        </h1>
      </Link>
      <Link
